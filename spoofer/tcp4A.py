...
from utils.conf import VPsConfig
    ...
    with open(targetFile) as ifile:
        ...
        for line in tqdm.tqdm(lines):
            ...
            for seq in range(0, 1):
                iptcpPkt = raw(IP(src=vpInfo[observer]['privateAddr'], dst=target) / TCP(sport=port_list[1], dport=port, flags="A", seq=1001, ack=21324) / Raw(load=data))
